...
def main():
        ...
        if new_license == "":
            ...
            if len(licenses_verify) == N_VERIFY and len(set(licenses_verify)) == 1:
                if prev_license == new_license:
                    ...
                    plat = new_license
                    os.chdir(r"D:\My file\MCTE\ENG lvl4.2 Sem1-2021\FYP2\Final report\ANPR_FYP PROJECT\Data_Base GUI")
                    column_names = ["NAME", "PLATE"]
                    df = pd.read_csv("Registered.csv", names=column_names)
                    plates = df.PLATE.to_list()
                    if plat in plates:

                        print("Access accepted, open the barrier")
                        cv2.putText(img_original_scene, "Access accepted, open the barrier", (10, 30),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, bottomLeftOrigin=False)
                        break
                    else:
                        cv2.putText(img_original_scene, "car is not registered, Access is denied", (10, 30),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, bottomLeftOrigin=False)
                        print("car is not registered, Access is denied")
                        break
    ...
    os.chdir(r"D:\My file\MCTE\ENG lvl4.2 Sem1-2021\FYP2\Final report\ANPR_FYP PROJECT\Data_Base GUI")
    column_names = ["NAME", "PLATE"]    #define the headers
    df = pd.read_csv("Registered.csv", names=column_names) #read the database
    plates = df.PLATE.to_list() #convert plates into list
    if plat in plates:  # Validation occurs
        print("Access accepted, open the barrier")
    else:
        print("car is not registered, Access is denied")
